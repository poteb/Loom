...
export const REACTION_TABLE = [
  ...
  "| `request.overdue` (a request you opened) | An accepted agent missed its deadline. Check its `lastSeenAt` with `get_request(requestId)`, call `remove_participant` with the request's `threadId` and that agent's `participantId`, then `accept` another standing offer with a new `deadlineMs`, or `open_request` anew. If every other agent you accepted has already completed, the removal closes the request as `completed` and you are done. |",
].join("\n");
