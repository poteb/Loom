...
(atomic claim + publish), and a writer that lost the race re-applies its change to the fresh state, so
concurrent channel processes never lose each other's updates. The state dir must be on a filesystem
