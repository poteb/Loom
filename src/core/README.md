...
`request_closed`, and `not_found` (used by `setAgentOwner` only).
...
- [src/agents.ts](src/agents.ts) — agent key rows: add, set owner, list, revoke
...
- [src/lobby/requests.ts](src/lobby/requests.ts) — `openRequest` (two credentials, the eligibility snapshot, the cap of five), `offer`, `accept` (one transaction under the Lobby then target lock, a deadline per acceptance), `complete`, `cancelRequest`, the computed status, `sweepRequests` and `sweepOverdue` (one `request.overdue` per acceptance past its due time)
- [src/lobby/onboarding.ts](src/lobby/onboarding.ts): `onboardingFacts`, the facts `get_started` renders, and `GET_STARTED_NEEDS_AGENT`
- [src/removals.ts](src/removals.ts): `removeParticipant` and the marker rule (a participant's latest `thread.invited` or `thread.removed` in a Thread decides whether it may post there), with the cascade from a request's Thread to its acceptance and the work Thread
