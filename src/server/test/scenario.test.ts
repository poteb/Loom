...
describe("v1 success scenario", () => {
  it("Claude Code creates a Weave, ChatGPT joins via /mcp, they collaborate, the Weave is archived", async () => {
    const stateDir = mkdtempSync(path.join(tmpdir(), "loom-scn-"));
    let claude: Client | undefined;
    let gpt: Client | undefined;
    let stderrBuf = "";
    try {
      // Claude Code side: the channel plugin over stdio
      claude = new Client({ name: "claude-code", version: "1" });
      ...
      gpt = new Client({ name: "chatgpt", version: "1" });
      ...
      const openerEvent = events.find((e: { type: string }) => e.type === "message");
      expect(openerEvent).toBeDefined();
      expect(openerEvent.payload.text).toContain("pull/42");
