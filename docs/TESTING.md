...
As of the **live instance** on `feat/live-instance` (measured at code commit `8f9995a`; the later
`f1e2496` changes only `deploy/test/`, which no vitest suite reads):
**1807 tests in 68 files** — core 562 in 25, web 749 in 14, server 208 in 10, claude-channel 139 in
...
cli 70/5, client 45/4, mcp-tools 34/1), so the live instance added **107 tests and two files**.
**Only `core` and `server` moved, and that is the measurement rather than the arithmetic**: core
gains `migration-status.test.ts` (one file, +77 with the db-guard rewrite, which adds tests to an
...
**28 cases, 27 passed, 0 failed, 1 skipped** on Windows (at `f1e2496`), exit 0. It is a `bash` runner
