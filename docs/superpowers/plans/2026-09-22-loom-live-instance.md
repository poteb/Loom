...
```
- [ ] **Step 8: `deploy/connector-url-to-clipboard.ps1`**, exactly as spec §4.7 gives it. It is run by **Paw**, because it puts something on his clipboard and he is the one about to paste it, and it prints **one line and not the URL**:
```powershell
$ErrorActionPreference = "Stop"
Set-StrictMode -Version Latest
...
**Hand-run steps go to Paw one at a time, with the real ids and secrets already substituted, waiting for each result before the next is sent** (`HANDBOOK.md` §4, `CLAUDE.md`). Never send him a batch.
