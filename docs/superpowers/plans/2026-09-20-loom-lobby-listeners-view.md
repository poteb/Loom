...
- **The history rules.**
  ...
  - The push is made **in `WeaveMount`**, which already computes that permission once as `canLeave` and already holds `storage`, `notice` and the Weave id it is computed from (`WeaveRoute.tsx:134-135`); it wraps the setter and gives `WeaveView` the wrapped `onView`. No new prop, and nothing below it has a history decision of its own.
...
```
  ...
  - **Test 16 — a mutation failure is visible while the directory is open.** `mountLobby({ path: "/lobby" })` and `await v.toggle()`. Two tests: a failed `createThread` from the sidebar (`[THREADS]: fail("internal", "boom", 500)`), and a failed guidelines save, each put their message in `.error-bar` with the directory on screen.
...
Spec §4.2 (with its 2026-09-20 amendment), §4.4, §4.5, §16 assumptions 4, 5, 9, 12, 17. **Tests 4, 5, 7, 8, 18, and the two this task adds of its own — 4b, the push that must not happen, and 4c, the permission re-read after an await.** Test 17 is Task 2's: it needs no history code, and the re-verification under Step 1 says so test by test.
...
**Interfaces:**
- *Consumes:* `viewOfPath`, `pathForView`, `MainArea` (Task 2); `leavingIsSafe` and `weaveKey`, already imported by `WeaveRoute.tsx` (`:6-7`) and already called there as `canLeave` (`:134-135`); `useRef`, added to the `preact/hooks` import at `:1`; `WeaveView`'s `view`, `viewKey` and `onView` props (Task 2).
- *Produces:* no new exported symbol. `WeaveSession`'s state widens from `MainArea` to `{ view: MainArea; popSeq: number }`, and `WeaveMount` passes `viewKey={popSeq}` and a **wrapped** `onView` that owns all three halves of the push rule.
...
- [ ] **Step 2: Run the tests to verify they fail**
  ...
  Expected: FAIL — `pushState` is never called, and `pop()` leaves the view where it was. **Three of these tests are red in two stages, and the second stage is not optional.** 4b's first two ("selecting a Thread pushes nothing", "creating one pushes nothing") and 4c pass trivially against Task 2's code, because nothing pushes there at all; what they are written against is the **unguarded** handler — `if (path && canLeave) push` with no change test and no re-read. So after Step 3 compiles, comment the two guards out for one run (or write the naive version first, which is the honest order), confirm those three go **red** with the message they are for — a duplicate `/lobby` for an ordinary Thread click, and a push made on a permission that expired inside an await — and put the guards back. A test that has never been seen to fail is not evidence.
...
```
  and hand `viewKey` and `onView` to `WeaveView`. Nothing below `WeaveMount` changes. `canLeave` keeps its own reader — the `HomeLink` of the no-credential fork and `openMainInPlace`, which are read during a render and are right to be — so this task adds a second call of `leavingIsSafe`, deliberately, rather than moving the first. One case a reviewer will look for: a join remounts `WeaveMount` under `key={reloadKey}`, so a callback captured before the join reads the **retired** mount's ref. It is harmless, and for the reason the state's home was chosen — `view` lives above that key and is the same value on both sides of the join, so the retired ref mirrors the live state, and `setView` is `WeaveSession`'s own and still live.
- [ ] **Step 4: Run the tests to verify they pass**
...
- [ ] **Step 1: Write the failing tests** in `src/web/test/listeners-page.test.tsx`:
  ...
  - **Test 11 — Clear filters.** `mountLobby` throughout, on its default `/lobby/listeners` or on a link of its own, and never through `toggle()`: two of these five read the address bar, which is only this page's where this page was loaded. Five tests: it is **rendered** at defaults and `disabled` there; one keystroke enables it **before** the 250 ms debounce fires (fake timers, `settleFake` as the block's settling helper, then advance 0); a single typed **space** enables it (the raw draft, not a trimmed one); pressing it with `sort=owner&dir=desc` in the link resets the two selects to `name`/`asc` **and** sends a query carrying neither (asserted on the **last** entry of `v.queries()`, not on a total); and pressing it leaves the address bar at a bare `/lobby/listeners` with `location.search === ""`.
- [ ] **Step 2: Run the tests to verify they fail**
...
- **§4.1** `routeOf` and `lobby-view.ts` → Task 2 Steps 5, 6, test 1. **§4.2** the push, with its 2026-09-20 amendment — only a real change, decided from current values → Task 3 Step 3, tests 4, 4b, 4c, 7, 8, 18. **§4.3** seeding and `writeSearch` → Task 2 Steps 11, 12. **§4.4** `popstate` and `popSeq` → Task 3 Step 3, test 5. **§4.5** the narrow exception → Task 2 Step 12's rewritten comment and Task 3's test 18, which is the pair of cases the path test alone got wrong.
