...
- [ ] **Step 3a: The transport, measured — not branched on.** Step 3b is the only test in this repo that renders a DOM **and** talks to a real server, so which `fetch` and which `WebSocket` it gets are questions about vitest's happy-dom environment. They are not open questions any more: they were **measured by the controller in the main checkout on 2026-09-20** — vitest 4.1.11, the happy-dom environment, a real `startTestServer()`, three probe tests, three passes — and the answers are written down here so that Step 3b carries one transport and no alternatives.
  **Read first, and true of the code as it stands:** `LoomClientOptions` is `{ baseUrl, token?, allowInsecure?, fetch? }` (`src/client/src/client.ts:10`) — `fetch` is injectable and there is **no** WebSocket option; `request` falls back to `globalThis.fetch` when none is given (`src/client/src/http.ts:33`); `session.ts:735` calls `reader.stream(weaveId, { since, onEvent, onStatus })` and passes **no** `WebSocketImpl`, so `openStream` takes `globalThis.WebSocket` (`src/client/src/stream.ts:13`, `:20`) — which is exactly why `session.test.ts:444-457` can count a session's sockets by swapping that global.
  **Measured, and what follows from it:**
  1. **happy-dom's `fetch` is same-origin.** Under `// @vitest-environment happy-dom`, a `new LoomClient({ baseUrl: s.baseUrl, allowInsecure: true })` call to the real server fails by default: `LoomClientError: Could not reach Loom: Cross-Origin Request Blocked: The Same Origin Policy disallows reading the remote resource at "http://127.0.0.1:<port>/api/..."`. The test page's default origin is not the server's, and happy-dom enforces the difference — so `allowInsecure: true` is necessary and is not sufficient.
  2. **One line settles both halves.** In `beforeAll`, immediately after `s = await startTestServer()`:
```ts
(window as unknown as { happyDOM: { setURL(u: string): void } }).happyDOM.setURL(s.baseUrl + "/lobby");
```
     With the page's origin made the server's and **nothing else changed**, the ambient `fetch` reaches the real server — `getInstanceGuidelines()` and `createWeave(...)` both answered — and the ambient `globalThis.WebSocket` opens a real stream: a session built with `createSession({ client, target: { kind: "secret", secret }, storage: memoryStorage() })` reached `getState().connection === "open"` inside the wait. So Step 3b brings **no** transport of its own, adds **no** dependency and swaps **no** global: its clients are plain `new LoomClient({ baseUrl: s.baseUrl, allowInsecure: true })`, the one `fetch` it passes is a **counter** that delegates straight to the ambient one (`mountLobby` below, which has to tell the directory's query from the count read), and the session takes the socket it already takes.
  3. **happy-dom prints a failed HTTP response to the console** — the measured run printed `GET http://127.0.0.1:<port>/api/lobby 404 (Not Found)` while no Lobby existed yet. This repo demands pristine test output, so Step 3b's fixture must leave **no request answering 4xx or 5xx during a normal run**, and that is a property of the fixture rather than a hope: with `await s.core.ensureLobby()` in `beforeAll` and a joined Lobby identity in storage, every request the mounted `<App/>` makes at `/lobby` answers 2xx. Each is named so a reviewer can tick it off rather than trust the list: `GET /api/lobby`, made by `LobbyRoute`'s own discovery and by the session (`client.ts:107`) — this is the one that 404s without `ensureLobby`, which is why that call is in `beforeAll` and not in the fixture; `GET /api/weaves/<lobbyId>` and `GET /api/weaves/<lobbyId>/events` (`:43`, `:55`); `GET /api/guidelines` (`:94`), served at `app.ts:63` and needing no credential; the four `GET /api/requests?status=...` the board reads, one open and three closed (`:160`, `session.ts:260-263`); `GET /api/lobby/listeners?limit=0&facets=false`, the session's count read (`:138`, `session.ts:350`); `GET /api/lobby/participants/me` (`:143`, `session.ts:315`), which answers 2xx because this storage carries a **joined identity** and not a secret; `POST /api/auth/ws-ticket` (`:189`); and the directory's own `GET /api/lobby/listeners?limit=50&...`. No test in Step 3b provokes a failing request, so none of them silences a line. Were one ever to need to — a test about a refused read — it would silence exactly that line and no other, with a `const err = vi.spyOn(console, "error").mockImplementation(() => {})` installed in that test and restored in it, never file-wide: a blanket silence would hide the 404 that a forgotten `ensureLobby` prints for every other test in the file.
  4. **The page's URL is real, and this file uses that.** Because the document genuinely sits at `http://127.0.0.1:<port>/lobby`, `location.pathname` reads `/lobby`, `viewOfPath` recognises it, and `history.pushState` to `/lobby/listeners` is same-origin and allowed — so the router, `viewOfPath` and the push of §4.2 run for real here rather than being worked around. Setting the URL again per test is also how this file resets `location` between tests: `beforeEach(() => setURL("/lobby"))`, where the stubbed suite uses `history.replaceState(null, "", "/")` (Global Constraints, happy-dom history).
...
/**
 * The page's own URL, and the whole of this file's transport (Step 3a). happy-dom's `fetch` is
 * same-origin, so a document left on its default origin cannot reach the server at all; given the
 * server's origin it reaches it, and `globalThis.WebSocket` opens a real stream to it. Nothing here
 * injects a `fetch` and nothing swaps a global.
 */
...
function mountLobby(f: Awaited<ReturnType<typeof lobbyFixture>>) {
  ...
  const client = new LoomClient({
    ...
    // (Step 3a). The **directory's** queries are told from the session's count read by `limit=0`,
    // exactly as the stubbed suite tells them apart (Step 1's `isCount`): they share a pathname, and
    // a counter on the pathname alone would move on every refresh and prove nothing.
    fetch: (input, init) => {
...
```
  ...
  **Its RED, honestly.** Against `main`'s code there is no directory inside the layout at all — `ListenersLink` is an anchor to `/lobby/listeners`, and no `<h2>Listeners</h2>` is rendered anywhere — so all three fail inside `openDirectory`, at `fireEvent.click(v.line()!)`, because `queryByRole("button", { name: /^Listeners/ })` is `null`. That is the right RED for the task whose code puts the directory in the layout, and it is unambiguous because the transport is not in question: Step 3a measured it, so a failure here is the feature's and never the environment's. The one failure that would say otherwise is named so it is recognised rather than debugged: a cross-origin `LoomClientError` from the very first request means `setURL` did not run, not that the page is broken.
...
```
  `-A src/web` covers the new `lobby-view-live.test.tsx`, which is the only file this task creates — no dependency is added and no lockfile moves, because Step 3a's measurement left this file with nothing to install. Then `git show --stat HEAD` and confirm no file is reported as `Bin`.
