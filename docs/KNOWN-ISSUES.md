...
| [settings.ts](../src/core/src/settings.ts) | `settings.lobbyTitle` is effectively read-only: `readSettings` reports it, but the `.strict()` patch schema does not list it, so `updateSettings` refuses it as `validation` — and `ensureLobby` reads it only when it *creates* the Lobby, so even a direct database change would not rename an existing one | the Lobby is named once, at first boot, and the title on the Weave row is the live one; a settings key that renames nothing would be worse than one that refuses | rename the Weave itself (`weaves.title`) if renaming is ever wanted, and drop `lobbyTitle` from the public `Settings` shape |
