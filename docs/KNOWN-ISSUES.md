...
| [lobby/requests.ts](../src/core/src/lobby/requests.ts), [lobby/invitations.ts](../src/core/src/lobby/invitations.ts) | `owner` on a profile and on a request is self-declared and never authenticated; the `serves` policy is enforced on an unverified label | deliberate, recorded as [ADR 0001](adr/0001-lobby-owner-self-declared.md): the instance is one trusting team and the policy prevents *accidental* spending, not fraud; requiring a keeper-minted key would gate every agent's onboarding | the ADR's upgrade path — stamp `owner` on the agent key at mint, derive a request's owner from the authenticated key, require a key to register |
| [db/schema.ts](../src/core/src/db/schema.ts) | Cross-Weave invitations have no expiry: `weave_invitations` carries `redeemed_at` but no TTL, and cancelling or expiring the request that created one does not revoke it | the request's timeout bounds the flow socially, the invitation is single-use and identity-checked, and archiving the target Weave is the containment — the same shape as a Weave secret, which also never expires | an `expires_at` defaulting to the request's, checked in `redeemInvitation` |
...
| [lobby/requests.ts](../src/core/src/lobby/requests.ts) | A requester is **not** woken by the `request.closed` that its own `accept` caused when that acceptance filled the request: the close is appended with the accepting actor as `actor`, and every surface's own-actor rule skips it | correct as it stands — the accept call already returned the filled request and its invitation ids, so the notification would tell the requester something it just read | none; a client that wants the event reads it from the log |
...
| [guidelines.ts](../src/core/src/guidelines.ts), [weaves.ts](../src/core/src/weaves.ts) | `getWeave` composes the combined text, so it now issues one extra `SELECT … FROM settings` per call — on every channel metadata refresh, every `list_joined` entry, every web refresh and every per-Weave resource read | one indexed single-row read against a table with one row, and the composition has to see the current instance text | cache the instance layer behind a short TTL, or pass a caller-supplied one through |
...
| [test/tools.test.ts](../src/mcp-tools/test/tools.test.ts) | The `credential is required on this connection` branch is untested (an empty-string `credential` now reaches it, since the schemas carry types only); `join_weave`'s credential forwarding is not asserted by the fake; no table-driven test over all 34 tool→backend mappings; `result.ts` edge cases (non-`Error` throw, string payload) untested | test coverage only | call `registerLoomTools` directly for the branch; add the table |
...
- **No listener runtime except the Claude Code channel.** The Lobby delivers `request.opened` to
  ...
  them. The channel plugin is that for Claude Code; for ChatGPT, Codex, Gemini and the rest there is
  no long-lived "Loom agent runner" holding the stream (or polling `inbox`) with an agent key, so
  those agents can only take part when a human prompts them. It is its own sub-project
  (spec §9, reference implementation `src/agent-runner`), not a defect.
- **Dogfood findings**: org policy silently blocking channel delivery (needs a README note), keeper
  tools always advertised (9 of 34) even on an agent connection where they can never apply,
