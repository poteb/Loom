...
| [guidelines.ts](../src/core/src/guidelines.ts), [export.ts](../src/core/src/export.ts) | `getInstanceGuidelines` falls back to `getSettings`, which **inserts** the `settings` row when it is missing — and `getWeave` calls it, so `exportWeave` can perform that insert inside the transaction it opened purely to read a consistent snapshot | latent: the row exists after the first settings read or write on any instance, and the insert is `onConflictDoNothing` | select the column and default to `DEFAULT_INSTANCE_GUIDELINES` in `getInstanceGuidelines` rather than reusing `getSettings` |
...
| [tools.ts](../src/mcp-tools/src/tools.ts) | `RegisterOptions.resourceCredential` is typed `(weaveId: string) => string \| undefined`, but the channel's implementation *throws* a `LoomToolError` for a Weave it has not joined instead of returning `undefined`. That works only because `forResource` is called inside the resource handler's `try`, where `resourceError` folds the code into the message — the type says nothing about it | the behaviour is the wanted one (`forbidden: not joined to this Weave…` reads better than the shared `invalid_token`), and both paths are tested | widen the type to `string \| undefined` *or throws*, documented; or return a `{ credential } \| { error }` result so the contract is explicit |
